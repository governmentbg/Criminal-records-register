...
namespace MJ_CAIS.Web.Controllers
{
    ...
    public class BulletinEventsController : BaseApiCrudController<BulletinEventDTO, BulletinEventDTO, BulletinEventGridDTO, BBulEvent, string>
    {
        ...
        [HttpGet("")]
        public async Task<IActionResult> GetAll(ODataQueryOptions<BulletinEventGridDTO> aQueryOptions, string typeCode)
        {
            var result = await this._bulletinEventService.SelectAllWithPaginationAsync(aQueryOptions, typeCode);
            return Ok(result);
        }
    }
}
