...
namespace EcrisServices
{
    public class EcrisFromCAISService
    {
        ...
        public async Task SendMessagesToEcris(string username, string password, string folderName, string endpointAuth, string endpointStorage, string endPointAddressSearch, int pageSize)
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    pageNumber++;
                    numberOfMessages = contents.Count();
                    if (contents.Count > 0)
                    {
